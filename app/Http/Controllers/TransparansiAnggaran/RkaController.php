...
class RkaController extends Controller
{
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    return
                        ...
                        <a href="' . route('rka.edit', $data->id) . '" class="btn btn-outline-primary rounded-round"><i class="far fa-plus-square mr-2"></i>Edit</a>
                        <a href="' . route('rka.destroy', $data->id) . '" class="btn btn-outline-danger rounded-round delete-data-table"><i class="fas fa-trash mr-2"></i>Hapus</a>
                    </div>';
                })
                ->rawColumns(['action'])
                ->make(true);
    ...
    public function store(Request $request)
    {
        TransparansiAnggaran::create([
            ...
            'nama' => 'RKA ' . $request->tahun
        ]);

        return redirect()->route('rka.index')->with('store', 'oke');
    ...
    public function update(Request $request, string $id)
    {
        ...
        return redirect()->route('rka.index')->with('edit', 'oke');
